# >> This file is not part of ratcam and is subject to a difference license,
# >> as it is meant to me merged into Picamera.
...
class OnlineMP4Muxer(object):
    def __init__(self, stream, framerate, resolution):
        super(OnlineMP4Muxer, self).__init__()
        self.stream = stream
    ...
    def _process_sps_header(self, sps_header_data):
        ...
        for nal_unit in nal_units:
            ...
            self.stream.write(struct.pack('>I', len(nal_unit))[-len(self.nal_prefix):])
            self.stream.write(nal_unit)
    ...
    def _output_mp4_header(self):
        self.stream.write(STATIC_FTYP)
        self.stream.write(STATIC_EMPTY_MDAT)


    def _patch_mdat_size(self):
        # Move to the position where the mdat size was
        self.stream.seek(self.mdat_offset)
        # Write the actual mdat size as big endian 32 bit integer
        self.stream.write(struct.pack('>I',
    ...
    def _patch_nal_sizes(self):
        for offset_in_mdat, size_to_write in self._nal_size_patches:
            self.stream.seek(self.mdat_payload_offset + offset_in_mdat)
            # Write the actual mdat size as big endian 32 bit integer
            self.stream.write(struct.pack('>I', size_to_write)[-len(self.nal_prefix):])
    ...
    def _output_mp4_footer(self):
        ...
        # Finally write
        self.stream.write(Box.build(MOOV))
